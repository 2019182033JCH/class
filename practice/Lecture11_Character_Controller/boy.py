...
# 이벤트 정의
RD, LD, RU, LU, AD, TIMER= range(6)  # RD, LD, RU, LU = 1, 2, 3, 4, 5
...
class IDLE:
    ...
    def do(self):
        ...
        if self.timer == 0:  # 시간이 다되면,
            self.add_event(TIMER)  # 타이머 이벤트를 큐에 삽입입        pass
...
class SLEEP:
    ...
    def draw(self):
        if self.face_dir == 1:
            self.image.clip_composite_draw(self.frame * 100, 300, 100, 100, 3.142592/2, '', self.x-25, self.y-25, 100, 100)
        else:
...
class AUTO_RUN:
    ...
    def draw(self):
        if self.dir == -1:
            self.image.clip_draw(self.frame * 100, 0, 100, 100, self.x, self.y, 100, 100)
        elif self.dir == 1:
            self.image.clip_draw(self.frame * 100, 100, 100, 100, self.x, self.y, 100, 100)
...
next_state = {
    ...
    AUTO_RUN: {RD: RUN, LD: RUN, AD: IDLE}
}
